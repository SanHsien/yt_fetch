...
- **Release maintenance**: cut a new tag and rebuild the EXE only when dependency freshness, real download issues, or core fixes make a new user-facing build worthwhile. The monthly dependency freshness workflow compares the repository's declared `yt-dlp` / `imageio-ffmpeg` baselines with PyPI, while weekly Dependabot checks every direct Python dependency and GitHub Action. Development and build tools directly exercised by required CI are automatically approved only after the full CI matrix, pre-commit, wheel build, and CodeQL pass; a global merge queue rebases them when needed, then squash-merges, closes, and removes their branches. Runtime dependencies, unexercised release tools, and GitHub Actions major updates keep a manual review gate. Each Release should list the bundled `yt-dlp` version, main changes, known limitations, and SHA256.
