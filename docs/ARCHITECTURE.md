...
| `.github/workflows/dependabot-merge.yml` | 全域序列化依賴 PR；必要時要求 rebase，重新驗證作者、政策 Check、head SHA、CI 與 CodeQL 後才核准／合併／關閉／刪分支 |
| `tools/classify_dependabot_update.py` | 純邏輯風險矩陣；CI 直接覆蓋才可自動處理，不確定時預設人工審查 |
